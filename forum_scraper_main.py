...
import os
from forum_scrape_utils import number_from_string, alpha_from_string
import time 
import datetime   
...
class forum_scraper:
    def __init__(self):
        ...
        self.user_safef_base = r'user_profile_data_n_'
        self.n_users = 88000
        self.form_data = {'username': 'forum_researcher','password': 'ForumResearcher1','redirect': './ucp.php?mode=login','sid': 'e1ba96ab85c15a9b2a7a5a1b7be552e8','redirect': 'index.php','login': 'Login'}
        self.login_post_url = r'https://www.14ers.com/forum/ucp.php?mode=login'
    ...
    def build_user_profile_dataframe(self):
        ##files aren't that large but wrote it this way to get still have intermittent saved
        ##files in case of python crash
        
        save_increment = 2000
        nsteps = self.n_users/save_increment
        for n in range(int(nsteps)):
            self.build_user_profile_dataframe_inner(3+n*save_increment,3+(n+1)*save_increment,n)
        self.reconstruct_single_user_df()
        
    def reconstruct_single_user_df(self):
        save_increment = 2000
        nsteps = self.n_users/save_increment
        fname_user = self.user_safef_base + str(0)
        df = pickle.load(open(fname_user,"rb"))
        print(nsteps)
        for n in range(int(nsteps)):
            fname_user = self.user_safef_base + str(n)
            print(fname_user)
            df_new = pickle.load(open(fname_user,"rb"))
            df = df.append(df_new,ignore_index=True)
        fname_user_full = r'user_profile_data_FULL_winterfix'
        with open(fname_user_full, 'wb') as handle:
            pickle.dump(df, handle)
                
    def build_user_profile_dataframe_inner(self,start,stop,index):
        ...
        with requests.Session() as sesh:
            ...
            for n in range(start,stop):
                ...
                if len(profile_info) > 0:
                    ...
                    try:
                        user_field_dict['Num14ers'] = int(profile_data[profile_fields.index('14erChecklist')].split('\xa0')[0])
                    except:
                        user_field_dict['Num14ers'] = np.nan
                    try:
                        user_field_dict['Num13ers'] = int(profile_data[profile_fields.index('13erChecklist')].split('\xa0')[0])
                    except:
                    ...
                    fname_user = self.user_safef_base + str(index)
...
#            fname_13er = r'13erChecklistByUser_df_winterfix'
...
#                pickle.dump(df_13erList, handle)    
            ...
            fname_14er = r'14erChecklistByUser_df_winterfix'
...
if __name__ == "__main__":
 ...
 #   fs.reconstruct_single_user_df()#build_user_profile_dataframe()
    fs.build_peakchecklist_dataframe()
